...
# Confirm that all NAs in `fledglings` column are real 0's --> change those
...
amoy$prod <- amoy$fledglings/amoy$pairs.b

# Writing outputs
write.csv(amoy, 'amoy.prod.csv', row.names=F)
...
# Does productivity differ amoung mainland vs. island sites?
ggplot(data = amoy,
       aes(x=year, y=prod))+
  geom_point()+
  facet_wrap(~type)
...
# stat='identity' to map y-aesthetic to a number (e.g. summary)
mean.amoy <- amoy %>%
  group_by(year) %>%
...
ggsave('amoy_mean.png', width=6, height=6, units='in', dpi=300)
ggsave('C:/Users/jwu/Documents/TrainingTutorials/R_for_Audubon/amoy_mean.pdf', width=6, height=6, units='in')
...
# run a shapiro test for productivity as a function of each 1) year, and 2) site type
...
## ------- Linear regression ------- ##
# Regression: simply relating one response variable to one or more independent variables
# predictors/independent variables can be continuous (e.g. productivity) or discrete (beach or island)

lm1 <- lm(prod ~ type, data=amoy2)
summary(lm1)
# being an island has a significant positive effect on productivity

lm2 <- lm(prod ~ acres, data=amoy2)
summary(lm2) 
# no effect of "acres"

lm3 <- lm(prod ~ miles.coastline, data=amoy2)
summary(lm3)
...
# check if residuals are normal. Obtain the standardized residual of the first linear model.
plot(lm1$residuals)
